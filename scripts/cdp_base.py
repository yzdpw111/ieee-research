...
def fetch_binary_diag(client, url, timeout=None):
    """页面上下文 fetch 二进制 + 诊断信息 → {data, status, contentType, finalUrl}"""
    timeout = timeout if timeout is not None else get("timeout.download")
    expr = (f"fetch({json.dumps(url)}).then(async r => {{"
            f"const ct = r.headers.get('content-type') || '';"
            f"const status = r.status; const finalUrl = r.url;"
            ...
            f"return {{status, contentType: ct, finalUrl, b64: btoa(s)}};}})")
    import base64
    r = client.evaluate(expr, await_promise=True, timeout=timeout) or {}
    data = base64.b64decode(r.get("b64")) if r.get("b64") else b""
    return {"data": data, "status": r.get("status"), "contentType": r.get("contentType"),
            "finalUrl": r.get("finalUrl")}


def fetch_binary(client, url, timeout=None):
    """页面上下文 fetch 二进制 → bytes（同源自动带 cookie，ieee 用）。"""
    return fetch_binary_diag(client, url, timeout)["data"]
